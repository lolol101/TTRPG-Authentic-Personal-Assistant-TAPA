...
## Запуск одним адресом (боевой режим)

Три сервиса выше — режим разработки: Vite раздаёт фронт и проксирует API.
Дев-сервер Vite не рассчитан на внешний трафик, поэтому наружу приложение
ходит иначе — бэкенд раздаёт собранный фронт сам, и адрес остаётся один:

```bash
cd apps/web-frontend && npm run build   # кладёт сборку в dist/
cd ../web-backend && uv run uvicorn app.main:app --port 8000
```

Открыть `http://localhost:8000/` — там и фронт, и API. Один origin означает,
что CORS не нужен вообще.

Сборку ищут рядом (`apps/web-frontend/dist`); переопределяется переменной
`FRONTEND_DIST_DIR`. Если сборки нет, бэкенд поднимается как обычно и отдаёт
только API — это и есть режим разработки, ошибкой не считается.

`llm-service` на порту 8100 нужен в обоих режимах.
