...
export function MainTab() {
  const { draft, sheet, setField, setNumberField, patchSheet, componentsFor, setComponents, abilityMod } =
    useSheet()
  ...
  function setRank(group: 'armor_proficiencies' | 'weapon_proficiencies', key: string, rank: ProficiencyRank) {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardContent className="space-y-3 pt-6">
              ...
              <div className="flex gap-1.5 pt-1.5">
                {[1, 2, 3].map((point) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      patchSheet({ hero_points: (sheet.hero_points ?? 0) === point ? 0 : point })
                    }
                    className={cn(
                      'size-8 rounded-full border-2 transition-colors',
                      point <= (sheet.hero_points ?? 0)
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            <Field label="Имя игрока" value={sheet.player_name ?? ''} onChange={(v) => patchSheet({ player_name: v })} />
            <Field label="Народ и родословная" value={draft.ancestry} onChange={(v) => setField('ancestry', v)} />
            <Field label="Родословная" value={sheet.heritage ?? ''} onChange={(v) => patchSheet({ heritage: v })} />
            <Field label="Происхождение" value={draft.background} onChange={(v) => setField('background', v)} />
            ...
            <Field label="Мировоззрение" value={sheet.alignment ?? ''} onChange={(v) => patchSheet({ alignment: v })} />
            <Field label="Дескрипторы" value={sheet.traits ?? ''} onChange={(v) => patchSheet({ traits: v })} />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <SectionTitle>Класс брони</SectionTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Field
                ...
                onChange={(value) =>
                  patchSheet({
                    armor: { ...armor, dex_cap: value === '' ? null : toNumber(value) },
                  })
                }
              />
              <Field
                ...
                type="number"
                value={armorPenalty}
                onChange={(value) => patchSheet({ armor: { ...armor, check_penalty: toNumber(value) } })}
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              <Field label="Бонус КБ" type="number" value={shield.ac_bonus} onChange={(v) => patchSheet({ shield: { ...shield, ac_bonus: toNumber(v) } })} />
              <Field label="Твёрдость" type="number" value={shield.hardness} onChange={(v) => patchSheet({ shield: { ...shield, hardness: toNumber(v) } })} />
              <Field label="Макс. ПЗ" type="number" value={shield.max_hp} onChange={(v) => patchSheet({ shield: { ...shield, max_hp: toNumber(v) } })} />
              <Field label="ПП" type="number" value={shield.bt} onChange={(v) => patchSheet({ shield: { ...shield, bt: toNumber(v) } })} />
            </div>
            <Field label="Текущие ПЗ щита" type="number" value={shield.current_hp} onChange={(v) => patchSheet({ shield: { ...shield, current_hp: toNumber(v) } })} />
          </CardContent>
        </Card>

        <Card>
          <CardHeader>
            <SectionTitle>Пункты здоровья</SectionTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Field label="При смерти" type="number" value={sheet.dying ?? 0} onChange={(v) => patchSheet({ dying: toNumber(v) })} />
              <Field label="Ранение" type="number" value={sheet.wounded ?? 0} onChange={(v) => patchSheet({ wounded: toNumber(v) })} />
            </div>

            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              <Field label="От народа" type="number" value={hp.ancestry} onChange={(v) => setHp('ancestry', v)} />
              <Field label="Класс за ур." type="number" value={hp.per_level} onChange={(v) => setHp('per_level', v)} />
              ...
              <Field label="Штраф" type="number" value={hp.penalty} onChange={(v) => setHp('penalty', v)} />
              <Field label="Скорость, фт." type="number" value={draft.speed} onChange={(v) => setNumberField('speed', v)} />
            </div>

            <p className="font-sans text-[11px] text-muted-foreground">
              {hp.ancestry} + ({hp.per_level} + {formatModifier(draft.con_mod)} ВЫН) × {draft.level} ур. = {maxHp}
            </p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <SectionTitle>Испытания</SectionTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <SectionTitle>Внимание и классовая СЛ</SectionTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            <StatRow
              label="Восприятие"
            ...
            />
            <Field label="Чувства" value={sheet.senses ?? ''} onChange={(v) => patchSheet({ senses: v })} />
            <Separator />
            <StatRow
              label="Классовая СЛ"
